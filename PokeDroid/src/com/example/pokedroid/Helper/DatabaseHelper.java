package com.example.pokedroid.Helper;
	...
	public List<String> getPokemonLocations(String pokemon){
		...
		try {
			...
			if (cursor.getCount() == 0) {
				...
				locs.add("Cannot be found in the wild!");
				return locs;
			}
	...
	public List<String> getPokemonMove(String name) {
		...
		try {
			...
			do {
				...
				pokemonNames.add(temp.substring(0, 1).toUpperCase() + temp.substring(1));
			} while (cursor.moveToNext());
	...
	public List<String> getEvolutionChain(String name) {
		...
		try {
			...
			if (cursor.getCount() == 0) {
                cursor.close();
                db.close();
				evolutionChain.add("No Evolutions!");
			...
			do {
	            temp = cursor.getString(0);
	            evolutionChain.add(temp.substring(0, 1).toUpperCase() + temp.substring(1));
	        } while (cursor.moveToNext()); 
